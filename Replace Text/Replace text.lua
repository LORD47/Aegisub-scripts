      ...
      while(i <= #t) do
        ...
        elseif(cmd.cmd_type == 'require')then
                ...
                if(tmp_module ~= nil)then
                   ...
                   for name, func in pairs(tmp_module)do
				      if(type(tmp_module['convert_units']) == 'function')then
				   ...
				   --aegisub.debug.out('tmp_module = "%s"\n', user_tools.modules[script_file]['convert_units'](5, 6)) -- tbr
				else
						  ...
						  if(user_tools.list[rules[j].func.name])then
							  ...
							  for _, param in ipairs(rules[j].func.args)do
								 ...
								 if(error_exists)then
									table.insert(invalid_commands, string.format('Error! motherfucker!'))
								 end
							  ...
							  if(not error_exists)then
								  local func_eval = user_tools.modules[user_tools.list[rules[j].func.name].file_name][rules[j].func.name](func_params)
...
function get_func_args(str)
    local pattern = '\\s*?([\\\\\\$]?\\d+|\\%(?:_?[a-zA-Z][a-zA-Z_0-9]*\\%)|(?:true|false)|[\'"].*?[\'"])(?:\\s*?,|$)'
	...
	while(matches ~= nil and tmp_str ~= str)do aegisub.debug.out('%sstr = %s\n', tmp_str and '\n' or '', str)-- tbr
	   ...
	   if(param.param_type)then
	      ...
	      tmp_str = str
	      str = str:gsub(matches[1].str:gsub("%p", "%%%1"), '', 1)
