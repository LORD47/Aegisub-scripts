...
function get_command_type(str)
  ...
  local cmds = { {pattern = '^\\%load\\s+(.+)', cmd_type = arr_cmd_types[1]},
                 ...
                 {pattern = '^\\%check\\s+m\\s*=\\s*([^;]+?)(?:;|;\\s*pp\\s*=\\s*\\[([^]]+?)\\]\\s*\\[([^]]*?)\\])?$', cmd_type = arr_cmd_types[7]},
      ...
      while(i <= #t) do
		...
		elseif(cmd.cmd_type == 'check')then
                ...
                if(cmd.matches[3] ~= nil)then
				    ...
				    if(cmd.matches[4] ~= nil)then -- this won't happen, but I added it now for the future
					     rule_class['pp'] = {str_match = trim(cmd.matches[3].str), str_replace = cmd.matches[4].str}

						 rule_class.pp.str_match = applyVars(rule_class.pp.str_match, vars_tbl, {fname = filename, line = t[i].line, full_str = t[i].str})
						 rule_class.pp.str_replace = applyVars(rule_class.pp.str_replace, vars_tbl, {fname = filename, line = t[i].line, full_str = t[i].str})
				    else 
...
function replaceText(i, idx, rules, line_txt, check_confirm, rplcd_at_lines, needs_conf, log_stats)
	 ...
	 for j = 1, #rules do
         ...
         elseif(check_confirm and rules[j].class == 'check')then	-- to manual edit in confirm pop-up		     
		     local tmp_checked_str = str

			 if(rules[j].check ~= nil)then -- has post-processing
			    --tmp_checked_str = re.sub(tmp_checked_str, rules[j].check.str_match, rules[j].check.str_replace)
			    tmp_checked_str = re.sub(tmp_checked_str, rules[j].check.str_match, '')
			 end
