	   ...
	   repeat
		  ...
		  for key, val in pairs(needs_conf) do
		    ...
		    for v, rule in pairs(val.rules) do		       
			   ...
			   else total_repls_to_review = total_repls_to_review - 1
					     ...
					     while(capture ~= nil and valid_args)do
						    ...
						    if(not error_exists)then
						       local current_func = {name = rules[j].func.name,
						  	   ...
						  	   aegisub.debug.out('\n------------------------------------------------------\n')-- tbr
						  	   aegisub.debug.out('current_func.data.args_type: %s\n\n', inspect(current_func.data.args_type, {newline = ''}) )-- tbr
						  	   aegisub.debug.out('rules[j].func.args: %s\n\n', inspect(rules[j].func.args, {newline = ''}) )-- tbr
							   ...
							   valid_args, args_error = valid_params(current_func.data.args_type, rule_func_args, current_func.data.optional, capture)
						  	   ...
						  	   aegisub.debug.out('func_params: %s\n\n', inspect(func_params, {newline = ''}) )-- tbr
						  	   aegisub.debug.out('------------------------------------------------------\n')-- tbr
                                  ...
                                  local func_eval = current_func.data.func(func_params)
						  	      ...
						  	      else table.insert(invalid_commands, func_eval.error)end
...
function valid_params(args_types, params, optional, captures)
   ...
   for i, param_types in ipairs(args_types)do
	  ...
	  aegisub.debug.out('param_types[%d]: %s\nparams[%d]: %s\ncaptures: %s\n', i, inspect(param_types), i, inspect(params[i], {newline = ''}), inspect(captures, {newline = ''}) )-- tbr
	       if(captures ~= nil and trim(params[i].param_type:lower()) == 'capture')then aegisub.debug.out('in capture\n' )-- tbr
		        
				if(captures[params[i].val] ~= nil)then
				    params[i].param_type = 'string'
				    params[i].val = captures[params[i].val]
				    
		           if(#param_types == 1 and trim(param_types[1]:lower()) == 'number')then aegisub.debug.out('params[%d].val = %s, %s\n', i, params[i].val, type(params[i].val) )-- tbr
